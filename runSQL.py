...
def run_query(nodeid, hostname, ip, query, queue):
        ...
        if data_str == "Failure":
            ...
            print (data)
        mySocket.close()
def db_runquery(hostname, ip, query, catnodes):
        port = 50001                        #port used for connection to server
        mySocket = socket.socket()          #creates a socket to allow for network connections
        mySocket.connect((ip,port))         #connects to server where query is being sent
        mySocket.send(query.encode())       #sends the query
        data = mySocket.recv(1024).decode()
        catnodes.append(data)
        mySocket.close()
...
#DDL statement that creates the catalog table
        createStatement = """CREATE TABLE IF NOT EXISTS DTABLES(tname char(32),
        nodeurl char(128), 
        partmtd int, 
        nodeid int, 
        partcol char(32), 
...
#for loop the parses the query, finds every instance of table and stores the tablename into the table array variable
        for x, word in enumerate(parse):    
            if word == "TABLE" or word == "INTO" or word == "FROM":
                firstsplit.append(parse[x + 1])
#for loop that completes the second part of the parse
        for x in firstsplit:
            split = x.split('(', 1)
            tables.append(split[0])
        tables[0] = tables[0].replace(";\n", "")
        db_query = """SELECT nodeid FROM dtables WHERE tname = '""" + tables[0] + """'"""
        db_runquery(cathost, catip, db_query, catnodes)
        catnodes[0] = str(catnodes[0])
        catnodes[0] = catnodes[0].replace("(", "")
        catnodes[0] = catnodes[0].split(",)")
        catnode = [y for x in catnodes for y in x]
        catnode = catnode[:-1]
#        if catnodes == []:
        ...
        for x in success:
            if x == 1:
                ...
                if parse[0] == "CREATE":
                    ...
                    updated = 1
                print (hostnames[count] + " " + sys.argv[2] + " " + "Success")
            count += 1 
